...
def ensure_table_exists(table: str, df: pd.DataFrame):
    """Create table with schema from df if it doesn't exist."""
    if df.empty:
        return
    conn = get_conn()
    cur = conn.cursor()
...
with tabs[0]:
    ...
    uploaded = st.file_uploader("Upload SeatMatrix (Excel/CSV)", type=["xlsx", "csv"])
    if uploaded:
        try:
            ...
            df = load_table("SeatMatrix", year, program)
            st.success(f"✅ Saved {len(df_new)} rows for {year}/{program}")
        except Exception as e:
            st.error(f"Error: {e}")

    st.dataframe(df, use_container_width=True)
    if st.button("🗑️ Flush SeatMatrix for this Year/Program"):
...
with tabs[1]:
    ...
    uploaded = st.file_uploader("Upload StudentDetails (Excel/CSV)", type=["xlsx", "csv"])
    if uploaded:
        try:
            if uploaded.name.lower().endswith(".csv"):
                df_new = pd.read_csv(uploaded)
    ...
    st.dataframe(df, use_container_width=True)
    if st.button("🗑️ Flush StudentDetails for this Year/Program"):
